                  ...
                  std::string frame_id)
{
    ...
    const float REFERENCE_ANGLE = 10.0;
    float last_known_deviation = 0.0;
    // Unpack data
    for (size_t i = 0; i < node_count; i++)
    {
        float corrected_angle = nodes[i].angle - last_known_deviation;
        while (corrected_angle < 0)
            corrected_angle += 360.0;
        while (corrected_angle >= 360)
            corrected_angle -= 360.0;
        size_t index = floor(corrected_angle);
        if (abs(index - REFERENCE_ANGLE) < 5 && scan_msg.ranges[index] != std::numeric_limits<float>::infinity())
        {
            last_known_deviation = nodes[i].angle - REFERENCE_ANGLE;
        }
...
int main(int argc, char *argv[])
{
    ...
    nh_private.param<string>("serial_port", opt_com_path, "/dev/robot_ttyUSB0");
